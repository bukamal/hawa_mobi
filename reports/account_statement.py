...
def _base_css(*, compact: bool, use_colors: bool = True) -> str:
    color_rules = ".debit{color:#1FA56A}.credit{color:#E54848}.balance{color:#0F766E}" if use_colors else ".debit,.credit,.balance{color:#172033}"
    return f"""
...
.report-header {{ border-bottom:3px solid #0F766E; padding-bottom:14px; margin-bottom:18px; position:relative; }}
.brand-block {{ display:flex; align-items:center; justify-content:center; gap:18px; text-align:center; }}
.brand-text h1 {{ margin:0; color:#0F766E; font-size:28px; line-height:1.35; }}
...
.account-badge {{ position:absolute; left:0; top:0; background:#F0FDFA; color:#0F766E; border-radius:12px; padding:9px 13px; font-weight:700; max-width:145px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }}
...
.account-line {{ font-size:16px; font-weight:700; color:#0F766E; margin-top:4px; }}
...
.note {{ border:1px solid #99F6E4; background:#F0FDFA; color:#0F766E; padding:11px 13px; border-radius:14px; margin:12px 0; font-size:13px; line-height:1.75; }}
...
.modern-table th {{ background:#0F766E; color:#fff; padding:10px 8px; border:1px solid #0F766E; font-size:13px; white-space:nowrap; }}
...
.idx { background:#F0FDFA; color:#0F766E; border-radius:999px; padding:3px 8px; font-weight:700; }
...
def export_service_profit_report_html(cases: Iterable[Dict], output_path: str | None = None) -> str:
    ...
    settings = get_report_settings()
    css = _base_css(compact=False, use_colors=bool(settings.get("statement_use_colors", True))) + "table{width:100%;border-collapse:collapse;min-width:980px}th{background:#0F766E;color:#fff;padding:8px;border:1px solid #0F766E}td{border:1px solid #d1d5db;padding:7px;font-size:12px;vertical-align:top}.summary-box{background:#F0FDFA;border:1px solid #99F6E4;border-radius:12px;padding:12px;margin:12px 0}"
