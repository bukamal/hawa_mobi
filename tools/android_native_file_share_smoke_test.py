...
assert "whatsapp_text_manual_file" not in share, (
    "WhatsApp file action must not auto-fallback to text-only wa.me"
)
assert "اختر واتساب من نافذة المشاركة لإرسال الملف" not in company, (
    "WhatsApp button must not send old instruction text"
)
assert 'message = f"كشف مطابقة - {self.company_name}"' in company, (
    "WhatsApp caption must be short and match the reconciliation report"
)
