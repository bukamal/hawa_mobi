...
def main() -> int:
    ...
    assert (
        FileExportService.resolve_picker_file_path(
            Picked(path=backup, name=os.path.basename(backup))
        )
        == backup
    )
    ...
    assert (
        FileExportService.resolve_picker_file_path(
            Picked(path=file_uri, name=os.path.basename(backup))
        )
        == backup
    )
    ...
    resolved = FileExportService.resolve_picker_file_path(
        Picked(name=os.path.basename(backup), size=os.path.getsize(target))
    )
    assert resolved == target, resolved

    # content:// must not crash when no Java bridge is available in desktop tests.
    unresolved = FileExportService.resolve_picker_file_path(
        Picked(
            uri="content://com.android.providers.downloads/document/123",
            name="missing.zip",
        )
    )
    assert unresolved is None
    details = FileExportService.describe_picker_file(
        Picked(uri="content://x", name="a.zip", size=7)
    )
