...
files = {
    "views/flet_compat.py": (ROOT / "views" / "flet_compat.py").read_text(encoding="utf-8"),
    "views/ui_kit.py": (ROOT / "views" / "ui_kit.py").read_text(encoding="utf-8"),
    "views/dialogs/dialog_kit.py": (ROOT / "views" / "dialogs" / "dialog_kit.py").read_text(encoding="utf-8"),
    "views/app_layout.py": (ROOT / "views" / "app_layout.py").read_text(encoding="utf-8"),
}
...
required = [
    ...
    ("ui_kit delegates to compat snackbar", "compat_show_snackbar" in files["views/ui_kit.py"]),
    ("dialog_kit delegates to compat snackbar", "compat_show_snackbar" in files["views/dialogs/dialog_kit.py"]),
    ("app_layout uses compat snackbar", "show_snackbar(self._page, message" in files["views/app_layout.py"]),
]
