...
assert "duration" in arg_names, "SettingsMobileView._show_snackbar must accept duration=..."
assert "return show_snackbar(self._page, message, is_error, duration=duration)" in settings, "settings snackbar wrapper must forward duration safely"

assert "def make_snackbar" in compat, "flet_compat must expose make_snackbar"
assert "_construct_with_keyword_fallback(ft.SnackBar" in compat, "SnackBar must be created via keyword fallback"
assert "snack.duration = duration" in compat, "duration should be set best-effort after construction"
assert "duration=duration" not in compat.split("def make_snackbar", 1)[1].split("def show_snackbar", 1)[0] or "_construct_with_keyword_fallback" in compat, "SnackBar duration must not be an unguarded hard dependency"
