...
def main() -> int:
    ...
    camera_service = (ROOT / "services" / "camera_permission_service.py").read_text(encoding="utf-8")

    assert "FilePicker(on_result" not in settings, "settings must not call FilePicker(on_result=...)"
    assert "make_file_picker" in compat and "attach_service_control" in compat, "compat helpers missing"
    assert "make_file_picker(self._on_restore_backup_picked)" in settings, "restore picker must use compat helper"
