...
def local_currency_and_reports(tmp: Path) -> None:
    ...
    assert "14,000" in rows[0]["historical_currency_value"], rows[0][
        "historical_currency_value"
    ]
    assert "300,000" in rows[0]["running_balance"], rows[0]["running_balance"]
    approx(totals["net_usd"], 20)
    statement = Path(export_account_statement_html("بلو ستار", [edited])).read_text(
        encoding="utf-8"
    )
    ...
    assert all(
        int(r.get("is_locked") or 0) == 1
        for r in client_rows + supplier_rows
        if r.get("source_ref") == result["reference"]
    )

    rec_html = Path(
        export_reconciliation_statement_html("بلو ستار", client_rows)
    ).read_text(encoding="utf-8")
    assert "كشف حساب للمطابقة" in rec_html
    assert "ليان الفيداوي" in rec_html
    assert (
        "تكلفة تأشيرة" not in rec_html
    )  # client reconciliation must not expose supplier-side cost wording
    profit_html = Path(export_service_profit_report_html(cases)).read_text(
        encoding="utf-8"
    )
    ...
    history = (
        __import__("database.connection", fromlist=["DatabaseConnection"])
        .DatabaseConnection()
...
def language_checks() -> None:
    ...
    assert key_sets["ar"] == key_sets["en"] == key_sets["fr"], (
        "language dictionaries must share the same keys"
    )
    for lang in langs:
...
def static_api_contract_checks() -> None:
    ...
    route_decorators = set(
        re.findall(
            r"""@app\.(?:get|post|put|delete)\(\s*["']([^"']+)["']""", server_text
        )
    )
    for route in [
        "/api/capabilities",
...
def live_api_checks(tmp: Path) -> None:
    try:
        ...
        print(
            "live Flask API test skipped: flask is not installed; static API contract passed"
        )
        return
    ...
    r = client.put(
        "/api/exchange_rates/SYP", json={"rate_to_usd": 14000}, headers=headers
    )
    ...
    eid = add.get_json()["id"]
    r = client.put(
        "/api/exchange_rates/SYP", json={"rate_to_usd": 16000}, headers=headers
    )
    assert r.status_code == 200, r.data
    ...
    search = client.get(
        "/api/search/company-ledger?q=مسافر%20API&limit=20", headers=headers
    )
