...
for path in ROOT.rglob("*.py"):
    ...
    assert not BAD.search(text), f"Use views.flet_compat alignment constants instead of enum-style ft.Alignment aliases: {path.relative_to(ROOT)}"

compat = (ROOT / "views" / "flet_compat.py").read_text(encoding="utf-8")
for token in ("ALIGN_CENTER", "ALIGN_TOP_LEFT", "ALIGN_BOTTOM_RIGHT", "patch_flet_alignment_aliases"):
