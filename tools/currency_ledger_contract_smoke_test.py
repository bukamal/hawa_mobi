...
def main():
    ...
    assert edited["exchange_rate_to_usd"] == 14000.0, (
        "same-currency edit must preserve historical rate"
    )
    assert edited["amount_base"] == 2.0

    changed_currency = svc.snapshot(2, "EUR", existing=edited)
    assert changed_currency["exchange_rate_to_usd"] == 0.92, (
        "currency change must capture a fresh snapshot"
    )
