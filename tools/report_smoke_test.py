...
def main() -> int:
    ...
    html_path = export_account_statement_html(
        "شركة اختبار", records, os.path.join(out_dir, "statement.html")
    )
    csv_path = export_account_statement_csv(
        "شركة اختبار", records, os.path.join(out_dir, "statement.csv")
    )
    html = Path(html_path).read_text(encoding="utf-8")
    assert "التاريخ" in html and "البيان" in html and "التراكمي" in html
    assert (
        "القيمة التاريخية للعملة" in html
    )  # folded into the statement meta in the mobile-safe layout
