...
def extract_server_routes() -> set[str]:
    ...
    routes = set(
        re.findall(r"@app\.(?:get|post|put|delete|patch)\(\s*[\"']([^\"']+)[\"']", text)
    )
    normalized = set()
    for route in routes:
...
def route_matches(endpoint: str, routes: set[str]) -> bool:
    ...
    return any(
        ...
        or re.fullmatch(
            re.escape(r).replace(re.escape("{var}"), r"[^/]+") + r"$", endpoint_path
        )
        for r in routes
    )
...
def assert_apk_safe() -> None:
    ...
    for token in forbidden:
        assert token not in pyproject, (
            f"APK pyproject must not package server dependency/module: {token}"
        )
    assert '"services*"' in pyproject, (
        "services package must be included for NetworkService imports"
    )
    ...
    for path in ROOT.rglob("*.py"):
        ...
        assert "flask" not in text.lower(), (
            f"APK-side module imports/refers to Flask: {rel}"
        )
        assert "waitress" not in text.lower(), (
            f"APK-side module imports/refers to Waitress: {rel}"
        )
...
def main() -> int:
    ...
    assert not missing, "Client endpoints without matching server route: " + ", ".join(
        missing
    )
