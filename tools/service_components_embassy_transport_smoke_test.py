...
try:
    ...
    result = service_repo.add({
        ...
        "components": [
            ...
            {"service_type": "سفارة / رسوم سفارة", "supplier_company_name": "رسوم سفارات", "sale_amount_original": 45, "cost_amount_original": 40},
            {"service_type": "نقل بري", "supplier_company_name": "شركة نقل الشام", "sale_amount_original": 25, "cost_amount_original": 20},
        ],
    })
    ...
    assert "تأشيرة سياحية" in client_rows[0]["print_description"], client_rows[0]["print_description"]
    assert "سفارة" in client_rows[0]["print_description"], client_rows[0]["print_description"]
    assert "نقل بري" in client_rows[0]["print_description"], client_rows[0]["print_description"]
    ...
    assert len(transport_rows) == 1 and float(transport_rows[0]["amount_original"]) == 20.0
    ...
    assert "سيف الشام" not in rec_html  # customer statement must not expose suppliers/profit
