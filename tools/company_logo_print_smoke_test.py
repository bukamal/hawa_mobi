...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="hawaa_logo_") as tmp:
        ...
        assert "branding" in stored, "logo must be copied into app-owned branding storage"
        assert image_to_data_uri(stored).startswith("data:image/png;base64,"), "logo must be embeddable as data URI"
        ...
        assert "file://" not in html, "print HTML must not reference private file:// logo paths"
        assert "class='company-logo'" in html or 'class="company-logo"' in html, "logo CSS class missing"
