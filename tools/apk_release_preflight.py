...
def main() -> int:
    ...
    if bad:
        return fail(
            "ملفات حساسة لا يجب أن تبقى في السورس: " + ", ".join(sorted(bad)[:20])
        )

    ui_kit = (ROOT / "views" / "ui_kit.py").read_text(encoding="utf-8")
    if "ft.Icons.FLIGHT" in ui_kit or "ft.Text('H'" in ui_kit:
        return fail(
            "ui_kit ما زال يستخدم شعار Android القديم H + Flight بدل شعار هوى الشام الموحد"
        )
    ...
    if "ft.ImageFit" in ui_kit:
        return fail(
            "ui_kit يستخدم ft.ImageFit؛ بعض إصدارات Flet داخل APK لا تدعمها. استخدم image_fit() بقيم نصية"
        )
    ...
    if bad_alpha:
        return fail(
            "ألوان alpha hex تسبب خللاً في Flet Android: " + ", ".join(bad_alpha[:10])
        )

    share_module = (ROOT / "reports" / "share.py").read_text(encoding="utf-8")
    if (
        "org.kivy.android.PythonActivity" in share_module
        or "from jnius import" in share_module
    ):
        return fail(
            "مشاركة Android لا يجب أن تعتمد على Kivy/pyjnius داخل Flet APK؛ استخدم Flet APIs مع fallback آمن"
        )
    required_share_terms = [
        'getattr(ft, "Share", None)',
        "copy_to_public_downloads",
        "manual_public_downloads",
        "manual_internal_path",
    ]
    missing_share_terms = [
        term for term in required_share_terms if term not in share_module
    ]
    if missing_share_terms:
        return fail(
            "مشاركة الملفات يجب أن تفحص وجود ft.Share فعليًا وتوفر fallback إلى Downloads/Hawaa: "
            + ", ".join(missing_share_terms)
        )
    if "ft.Share()" in share_module:
        return fail(
            'لا تستدع ft.Share() مباشرة؛ بعض APK runtimes لا تحتوي الخدمة. استخدم getattr(ft, "Share", None)'
        )
    file_export = (ROOT / "services" / "file_export_service.py").read_text(
        encoding="utf-8"
    )
    if (
        ".backup(" not in file_export
        or '"sqlite_snapshot": "backup_api"' not in file_export
    ):
        return fail(
            "نسخ SQLite الاحتياطي يجب أن يستخدم SQLite backup API حتى لا يفقد بيانات WAL"
        )
    if (
        "DELETE FROM settings WHERE key='auth/network_token'" not in file_export
        or '"credentials_included": False' not in file_export
    ):
        return fail(
            "النسخة الاحتياطية يجب أن تستبعد رمز جلسة الشبكة وتعلن عدم تضمين بيانات الاعتماد"
        )

    settings_view = (ROOT / "views" / "settings_mobile_view.py").read_text(
        encoding="utf-8"
    )
    if "سيتم تطبيق اللغة بعد إعادة التشغيل" in settings_view:
        return fail(
            "تغيير اللغة لا يزال يتطلب إعادة تشغيل؛ يجب تطبيقه فورياً عبر _hawaa_rebuild_main"
        )
    if (
        "_hawaa_refresh_current_page" not in settings_view
        or "currency.save_runtime_settings" not in settings_view
    ):
        return fail(
            "تغيير عملة العرض يجب أن يطبق فوراً عبر currency.save_runtime_settings و _hawaa_refresh_current_page"
        )
    currency_module = (ROOT / "currency.py").read_text(encoding="utf-8")
    if (
        "def invalidate_cache" not in currency_module
        or "def save_runtime_settings" not in currency_module
    ):
        return fail(
            "CurrencyManager لا يحتوي آلية تحديث إعدادات العملة فورياً بدون إعادة تشغيل"
        )
    settings_repo_module = (
        ROOT / "database" / "repositories" / "settings_repo.py"
    ).read_text(encoding="utf-8")
    if (
        "_shared_cache" not in settings_repo_module
        or "invalidate_cache" not in settings_repo_module
    ):
        return fail(
            "SettingsRepository يحتاج cache مشترك وإبطال cache لمنع بقاء عملة العرض القديمة"
        )
    ...
    leaked_permissions = [
        value for value in forbidden_android_permissions if value in pyproject
    ]
    if leaked_permissions:
        return fail(
            "APK يطلب أذونات تخزين/وسائط غير لازمة: " + ", ".join(leaked_permissions)
        )
    if (
        "[tool.flet.android.permission]" not in pyproject
        or '"android.permission.CAMERA" = true' not in pyproject
    ):
        return fail(
            "إذن الكاميرا يجب أن يعلن بصيغة Flet الحديثة داخل [tool.flet.android.permission]"
        )
    if 'permissions = ["camera"]' not in pyproject:
        return fail(
            "حزمة Flet يجب أن تطلب الكاميرا فقط؛ FilePicker يستخدم Storage Access Framework بدون إذن تخزين شامل"
        )
    ...
    if "resources/sounds" in pyproject:
        return fail(
            "هذا فرع Flet/Android لا يجب أن يرث resources/sounds الخاصة بسطح المكتب دون قرار واضح"
        )
    ...
    if (
        "NetworkService.save_mode(" not in pairing
        or '"client", server_url' not in pairing
    ):
    ...
    if "ربط مع Windows عبر QR" not in login_view:
        return fail(
            "واجهة تسجيل الدخول يجب أن توفر ربط Android مع Windows عبر QR قبل تسجيل الدخول"
        )
    if "ربط عبر QR" not in settings_view:
        return fail("إعدادات الشبكة يجب أن توفر ربط QR")
    camera_permission = (ROOT / "services" / "camera_permission_service.py").read_text(
        encoding="utf-8"
    )
    if (
        "PermissionHandler" not in camera_permission
        or "request_permission" not in camera_permission
    ):
    ...
    if "استخدم مسار الملف مباشرة" in settings_view:
        return fail(
            "اختيار شعار الشركة لا يزال يعتمد على مسار يدوي؛ يجب استخدام FilePicker ونسخ الشعار للتخزين الداخلي"
        )
    if (
        "make_file_picker" not in settings_view
        or "company_logo_service" not in settings_view
    ):
        return fail(
            "اختيار شعار الشركة يجب أن يستخدم make_file_picker و company_logo_service"
        )
    if "FilePicker(on_result" in settings_view:
        return fail(
            "لا تستخدم FilePicker(on_result=...)؛ بعض إصدارات Flet ترفضها. استخدم make_file_picker()."
        )
    if "make_file_picker" not in settings_view:
        return fail(
            "اختيار الملفات يجب أن يستخدم make_file_picker للتوافق مع إصدارات Flet المختلفة"
        )
    logo_service = (ROOT / "services" / "company_logo_service.py").read_text(
        encoding="utf-8"
    )
    ...
    if len(sys.argv) > 1:
        ...
        if artifact.suffix.lower() in {".zip", ".apk", ".aab"}:
            with zipfile.ZipFile(artifact) as zf:
                names = zf.namelist()
                forbidden_inside = [
                    n for n in names if any(t in n for t in FORBIDDEN_ARTIFACT_PATTERNS)
                ]
                if forbidden_inside:
                    return fail(
                        "الأثر يحتوي ملفات ممنوعة: " + ", ".join(forbidden_inside[:20])
                    )
