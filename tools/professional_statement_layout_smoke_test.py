...
assert 'class="movement"' in recon_html, (
    "Reconciliation must be card-based, not a wide 8-column table"
)
assert "الزبون / المسافر</th>" not in recon_html, (
    "External reconciliation must not use separate passenger column"
)
assert "الخدمة / البنود</th>" not in recon_html, (
    "External reconciliation must fold service into statement body"
)
assert "SVC-BFFAAB" in recon_html, (
    "Long service refs should be shortened for mobile/WhatsApp"
)
...
assert "+963" in recon_html or "963" in recon_html, (
    "Header contact should be present and isolated"
)
...
assert "Ref: SVC-20260712-235813-BFFAAB" in detail_html, (
    "Detailed report should preserve full reference"
)
assert "الزبون / المسافر</th>" not in detail_html, (
    "Detailed mobile table should be limited to core columns"
)

company_view = (ROOT / "views" / "company_details_mobile_view.py").read_text(
    encoding="utf-8"
)
