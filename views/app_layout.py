...
from views.ui_kit import app_brand, PRIMARY_SOFT, PAGE_BG, CARD_BG
...
class AppLayout(ft.Column):
    def __init__(self, page, on_logout=None):
        ...
        try:
            ...
            setattr(
                self._page, "_hawaa_refresh_current_page", self.refresh_current_page
            )
    ...
    def _refresh_status_bar(self):
        ...
        user_label = (
            current.get("full_name") or current.get("username") or translate("login")
        )
            ...
            try:
                ...
                overdue = len(
                    [
                        r
                        for r in reminders
                        if r.get("reminder_date") and r.get("reminder_date") < today
                    ]
                )
            except Exception:
                overdue = 0
    ...
    def _build_nav_bar(self):
        user_role = (
            UserSession.get_current().get("role")
            if UserSession.get_current()
            else "user"
        )
        destinations = [
            ft.NavigationBarDestination(
                icon=ft.Icons.DASHBOARD, label=translate("dashboard")
            ),
            ft.NavigationBarDestination(
                icon=ft.Icons.ACCOUNT_BALANCE, label=translate("accounts")
            ),
        ]
        if user_role == "admin":
            destinations.append(
                ft.NavigationBarDestination(
                    icon=ft.Icons.PEOPLE, label=translate("users")
                )
            )
            destinations.append(
                ft.NavigationBarDestination(
                    icon=ft.Icons.ASSIGNMENT, label=translate("audit_log")
                )
            )
        destinations.append(
            ft.NavigationBarDestination(
                icon=ft.Icons.SETTINGS, label=translate("settings")
            )
        )
    ...
    def switch_page(self, page_id):
        ...
        try:
            ...
            pass
        self._refresh_status_bar()
        self.content_area.content = loading_view(
            "جاري فتح الشاشة..."
            if translate("settings") == "الإعدادات"
            else "Loading screen..."
        )
        ...
        try:
            ...
            self.content_area.content = error_view(
                str(ex), on_retry=lambda e: self.switch_page(page_id)
            )
        safe_update(self._page)
    ...
    def _logout(self, e):
        ...
        def confirm_logout(e):
            ...
            if self.on_logout:
                ...
                login = LoginView(
                    page=self._page,
                    on_login_success=lambda u: self._rebuild_after_login(),
                    on_exit=self.on_logout,
                )
