...
class ActivationView(ft.Container):
    def __init__(self, page, on_success, on_cancel):
        ...
        self.status_chip = status_chip(
            "مفعل" if details.get("activated") else "غير مفعل",
            icon=ft.Icons.VERIFIED_USER,
            color=SUCCESS if details.get("activated") else DANGER,
            bgcolor="#E9F7F1" if details.get("activated") else "#FDECEC",
        )
        ...
        self.license_info = ft.Text(
            self._format_license_info(details),
            size=11,
            color=MUTED,
            text_align=ft.TextAlign.CENTER,
            selectable=True,
        ...
        self.device_text = ft.Text(
            f"معرّف الجهاز: {self.device_id[:12]}…{self.device_id[-8:]}",
            size=11,
            color=MUTED,
            selectable=True,
            text_align=ft.TextAlign.CENTER,
        ...
        self.activate_btn = ft.FilledButton(
            content=ft.Text("تفعيل", size=16, weight=ft.FontWeight.BOLD),
            width=360,
            height=46,
            bgcolor=PRIMARY,
        ...
        self.copy_btn = ft.OutlinedButton(
            content=ft.Text("نسخ معرّف الجهاز"), on_click=self._copy_device_id
        )
        self.refresh_btn = ft.OutlinedButton(
            content=ft.Text("تحديث الحالة"), on_click=self._refresh_status
        )
        self.cancel_btn = ft.TextButton(
            content=ft.Text("إغلاق", color=MUTED),
        ...
        form = ft.Column(
            controls=[
                app_brand(
                    "تفعيل هوى الشام", "ربط الترخيص بهذا الجهاز", size=88, dark=True
                ),
                self.status_chip,
                self.license_info,
                self.device_text,
                self.path_text,
                ft.Row(
                    [self.copy_btn, self.refresh_btn],
        ...
        self.content = brand_background(
            brand_card(form, width=450, padding=24), padding=18, dark=False
        )
    ...
    def _refresh_status(self, e=None):
        details = get_license_details()
        self.status_chip.content.controls[-1].value = (
            "مفعل" if details.get("activated") else "غير مفعل"
        )
