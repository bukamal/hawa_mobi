...
def _format_payload_summary(payload: str) -> str:
    try:
        ...
        token_hint = (
            (token[:6] + "…" + token[-4:]) if len(token) > 12 else (token or "—")
        )
    ...
    try:
        ...
        if loading:
            button.content = ft.Row(
                [ft.ProgressRing(width=16, height=16), ft.Text(text)],
...
def open_qr_pairing_dialog(page: ft.Page, *, on_success=None, initial_text: str = ""):
    ...
    summary_text = ft.Text(
        "", size=11, color=ft.Colors.BLUE_GREY_700, text_align=ft.TextAlign.RIGHT
    )
    ...
    try:
        ...
        allow_http_default = NetworkService.insecure_http_enabled()
    except Exception:
        allow_http_default = False
    allow_http_checkbox = ft.Checkbox(
        label="السماح باتصال HTTP غير مشفر داخل الشبكة المحلية فقط",
        value=allow_http_default,
    )
    ...
    apply_btn = ft.FilledButton(
        content=ft.Row(
            [ft.Icon(ft.Icons.LINK), ft.Text("ربط وحفظ")],
            alignment=ft.MainAxisAlignment.CENTER,
        ),
    ...
    def _apply_payload(payload: str):
        try:
            ...
            result = MobilePairingService.pair_from_qr_text(
                payload,
                allow_insecure_http=bool(allow_http_checkbox.value),
            )
            if not result.ok:
    ...
    def do_pair_code(ev):
        try:
            ...
            result = MobilePairingService.pair_with_code(
                server_url,
                code,
                allow_insecure_http=bool(allow_http_checkbox.value),
            )
            ...
            try:
                ...
                code_pair_btn.content = ft.Row(
                    [ft.Icon(ft.Icons.PIN), ft.Text("ربط بالرمز اليدوي")],
                    alignment=ft.MainAxisAlignment.CENTER,
                )
                page.update()
            except Exception:
    ...
    def scan_camera(ev):
        ...
        try:
            ...
            pass
        scanner_cls = (
            getattr(ft, "BarcodeScanner", None)
            or getattr(ft, "QrScanner", None)
            or getattr(ft, "QRScanner", None)
        )
    ...
    camera_btn = ft.FilledButton(
        content=ft.Row(
            [ft.Icon(ft.Icons.CAMERA_ALT), ft.Text("مسح بالكاميرا")],
            alignment=ft.MainAxisAlignment.CENTER,
        ),
    ...
    paste_btn = ft.OutlinedButton(
        content=ft.Row(
            [ft.Icon(ft.Icons.CONTENT_PASTE), ft.Text("لصق من الحافظة")],
            alignment=ft.MainAxisAlignment.CENTER,
        ),
    ...
    dlg = ft.AlertDialog(
        ...
        content=ft.Container(
            ...
            content=ft.Column(
                [
                    info_banner(
                        "الربط لا يمنح صلاحيات دخول. بعد نجاح الربط ستسجل الدخول باسم المستخدم وكلمة المرور.",
                        icon=ft.Icons.QR_CODE_SCANNER,
                    ),
                    camera_btn,
                    ft.Text(
                        "أو استخدم النص الاحتياطي",
                        size=12,
                    ...
                    ft.Divider(),
                    ft.Text(
                        "أو أدخل عنوان الخادم والرمز القصير الظاهر في Windows",
                        size=11,
                        color=ft.Colors.GREY_600,
                        text_align=ft.TextAlign.CENTER,
                    ),
                    ...
                    allow_http_checkbox,
                    info_banner(
                        "استخدم HTTP فقط مع عنوان IP خاص داخل شبكة موثوقة. للخوادم العامة يجب استخدام HTTPS.",
        ...
        actions=[
            apply_btn,
            ft.TextButton("إلغاء", on_click=lambda ev: close_control(page, dlg)),
        ],
