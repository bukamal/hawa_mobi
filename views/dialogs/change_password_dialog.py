...
class ChangePasswordDialog(ft.AlertDialog):
    def __init__(self, page, on_save=None, user_id=None):
        ...
        self.content = dialog_body(
            controls=[self.old_password, self.new_password, self.strength_text, self.confirm_password, self.match_text],
            spacing=15,
    ...
    def _validate_live(self, e=None):
        ...
        if new:
            ...
            self.strength_text.color = ft.Colors.GREEN if policy['ok'] else ft.Colors.ORANGE
        else:
    ...
    def _save(self, e):
        ...
        policy = evaluate_password(new)
        if not policy['ok']:
            self._show_snackbar('كلمة المرور ضعيفة: ' + '، '.join(policy['problems'][:3]), True)
            return
        if old == new:
