...
from views.ui_kit import (
    app_brand, brand_background, brand_card, status_chip, info_banner, show_snackbar,
    PRIMARY, PRIMARY_SOFT, MUTED, DANGER, SUCCESS, TEXT, BORDER, CARD_BG,
)
from views.design_system.responsive import form_width
...
class LoginView(ft.Container):
    ...
    def __init__(self, page, on_login_success, on_exit):
        ...
        field_width = min(430, form_width(page, 430))
        self.network_chip = status_chip(translate("local_mode"), icon=ft.Icons.PHONE_ANDROID, color=SUCCESS, bgcolor="#E9F7F1")
        self.network_status = ft.Text("", size=11, color=MUTED, text_align=ft.TextAlign.CENTER, max_lines=2, overflow=ft.TextOverflow.ELLIPSIS)

        self.username = ft.Dropdown(
            label=translate("username"), hint_text=translate("username"), options=[], editable=True,
            width=field_width, border_radius=12, filled=True, bgcolor=CARD_BG,
            border_color=BORDER, focused_border_color=PRIMARY, text_size=14,
        )
        self.password = ft.TextField(
            label=translate("password"), password=True, can_reveal_password=True,
            width=field_width, border_radius=12, filled=True, bgcolor=CARD_BG,
            border_color=BORDER, focused_border_color=PRIMARY, text_size=14,
            prefix_icon=ft.Icons.LOCK_OUTLINE,
        )
        self.password.on_submit = self._do_login
        self.error_msg = ft.Text("", color=DANGER, size=12, text_align=ft.TextAlign.CENTER, max_lines=3)
        self.login_btn = ft.FilledButton(
            content=ft.Text(translate("login"), size=15, weight=ft.FontWeight.BOLD),
            width=field_width, height=52, bgcolor=PRIMARY, color=ft.Colors.WHITE,
            on_click=self._do_login,
            style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=12)),
        )
        self.lang_dropdown = ft.Dropdown(
            label=translate("language"), width=field_width, value=language_label(),
            options=[ft.dropdown.Option("العربية"), ft.dropdown.Option("English"), ft.dropdown.Option("Français")],
            border_radius=12, filled=True, bgcolor=CARD_BG, border_color=BORDER,
        ...
        self.saved_password_hint = ft.Text(translate("remember_password_security_hint"), size=10, color=MUTED, text_align=ft.TextAlign.CENTER)
        self.brand = app_brand(translate("app_name"), translate("login_subtitle"), size=86, dark=True)
        self.form_title = ft.Text(translate("login_data"), size=18, weight=ft.FontWeight.BOLD, color=TEXT)
        ...
        self.qr_pair_btn = ft.OutlinedButton(
            content=ft.Row([ft.Icon(ft.Icons.QR_CODE_SCANNER, color=PRIMARY), ft.Text("ربط مع Windows عبر QR", color=PRIMARY, weight=ft.FontWeight.BOLD)], tight=True),
            width=field_width, height=48, on_click=self._open_qr_pairing_dialog,
            style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=12), side=ft.BorderSide(1, BORDER)),
        )
        ...
        connection_panel = ft.Container(
            content=ft.Column([self.network_chip, self.network_status], spacing=6, horizontal_alignment=ft.CrossAxisAlignment.CENTER),
        ...
        form = ft.Column(
            controls=[
                ...
                ft.Column([
                    self.form_title,
                    ft.Text("أدخل بيانات حسابك للوصول إلى النظام", size=12, color=MUTED),
                ], spacing=2, horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                self.username,
                self.password,
                self.error_msg,
                ft.Container(content=self.remember, width=field_width, alignment=ft.alignment.center_right),
                self.saved_password_hint,
                self.login_btn,
                ft.Row([ft.Container(content=ft.Divider(color=BORDER), expand=True), ft.Text("أو", size=11, color=MUTED), ft.Container(content=ft.Divider(color=BORDER), expand=True)], width=field_width),
                self.qr_pair_btn,
                self.lang_dropdown,
        ...
        card_width = min(480, max(300, form_width(page, 480)))
        self.content = brand_background(brand_card(form, width=card_width, padding=22), padding=16, dark=False)
