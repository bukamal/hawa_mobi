...
class UsersMobileView(ft.Column):
    def __init__(self, page):
        ...
        self.controls = [
            page_header(
                "إدارة المستخدمين", ft.Icons.GROUPS, subtitle="صلاحيات الدخول والحسابات"
            ),
            self.summary,
            self.users_list,
        ]
    ...
    def _load_users(self):
        try:
            ...
            for u in users:
                        ...
                        [
                            ft.Row(
                                [
                                    ft.Container(
                                        content=ft.Icon(
                                            role_icon, color=role_color, size=22
                                        ),
                                    ...
                                    ft.Column(
                                        [
                                            ...
                                            ft.Text(
                                                u.get("full_name") or "بدون اسم كامل",
                                                size=12,
                            ...
                            ft.Row(
                                [
                                    action_text_button(
                                        "تعديل",
                                        ft.Icons.EDIT,
                                        lambda e, uid=u["id"]: self._edit_user(uid),
                                    ),
                                    action_text_button(
                                        "حذف",
                                        ft.Icons.DELETE,
                                        lambda e, uid=u["id"]: self._delete_user(uid),
                                        color=ft.Colors.RED,
                                        visible=(u.get("id") != 1),
                                    ),
                                ],
                ...
                cards.append(card)
            self.users_list.controls = cards or [
                empty_state(
                    "لا يوجد مستخدمون",
                    "استخدم زر الإضافة لإنشاء مستخدم جديد",
                    ft.Icons.GROUP_OFF,
                )
            ]
            self._page.update()
    ...
    def _edit_user(self, user_id):
        ...
        dialog = UserDialog(
            page=self._page, user_id=user_id, on_save=lambda: self._load_users()
        )
        open_control(self._page, dialog)
