...
def loading_view(message: str = "جاري التحميل..."):
    return ft.Container(
        content=ft.Column(
            [ft.ProgressRing(width=26, height=26), ft.Text(message, size=13, color=ui_kit.MUTED)],
...
def error_view(message: str, on_retry=None):
    controls = [
        ...
        ft.Text(str(message), size=12, color=ui_kit.MUTED, text_align=ft.TextAlign.CENTER),
    ]
    if on_retry:
...
def network_status_chip():
    try:
        ...
        if db.is_remote():
            ...
            return ui_kit.pill(f"{label} • {subtitle}", color=ft.Colors.BLUE, bgcolor=ft.Colors.BLUE_50, icon=ft.Icons.LAN)
        return ui_kit.pill("محلي", color=ft.Colors.GREEN, bgcolor=ft.Colors.GREEN_50, icon=ft.Icons.STORAGE)
    except Exception:
        return ui_kit.pill("حالة الشبكة غير معروفة", color=ft.Colors.ORANGE, bgcolor=ft.Colors.ORANGE_50, icon=ft.Icons.WARNING_AMBER)


def make_status_bar(user_label: str = ""):
    left = ft.Row([
        ft.Icon(ft.Icons.VERIFIED_USER, size=16, color=ui_kit.PRIMARY),
        ft.Text(user_label or "جلسة نشطة", size=12, color=ui_kit.MUTED, overflow=ft.TextOverflow.ELLIPSIS),
