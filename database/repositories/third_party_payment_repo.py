...
class ThirdPartyPaymentRepository(BaseRepository):
    ...
    def _expense_payload(
        ...
        *,
        ...
        data["is_locked"] = 1
        normalized = normalize_expense_metadata(
            self.ledger.normalize_expense_payload(data)
        )
    ...
    def add_payment_on_behalf(
        self,
        payer_company_name: str,
        paid_to_company_name: str,
        amount: float,
        currency_code: str,
        date: str,
        notes: str = "",
        ...
        paid_to = self._normalize_company(
            paid_to_company_name, "الشركة التي تم السداد لها"
        )
        if payer == paid_to:
    ...
    def reverse_payment_on_behalf(
        self, reference: str, user_id: int | None = None, date: str | None = None
    ) -> Dict:
        ...
        try:
            ...
            conn.execute(
                ...
                (datetime.datetime.now().isoformat(), reversal_ref, reference),
            )
            conn.execute(
                "INSERT INTO audit_log (user_id, username, action, table_name, record_id, details, ip_address, timestamp) VALUES (?,?,?,?,?,?,?,?)",
                (
