    ...
    def add(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if supplier_expense_id and float(payload.get("supplier_paid_amount") or 0) > 0:
                ...
                insert_payment_in_transaction(conn, target, payload.get("supplier_paid_amount"), date=payload["date"], payment_method=payload.get("payment_method") or "cash", notes="دفعة أولى للمورد", payer_type="office", payer_name="المكتب", user_id=uid, username=user.get("username", ""), now=now)
