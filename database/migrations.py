...
def _ensure_local_notification_schema(cursor):
    ...
    cursor.execute("INSERT OR REPLACE INTO settings(key,value) VALUES('schema_version','27')")
...
def init_database():
    ...
    cursor.executescript('''
        ...
        CREATE TABLE IF NOT EXISTS payments (id INTEGER PRIMARY KEY AUTOINCREMENT, reference TEXT UNIQUE NOT NULL, target_expense_id INTEGER NOT NULL, company_name TEXT NOT NULL, person_name TEXT, source_type TEXT, source_ref TEXT, party_role TEXT, amount_original REAL NOT NULL, currency_original TEXT NOT NULL, exchange_rate_to_usd REAL NOT NULL DEFAULT 1.0, amount_base REAL NOT NULL DEFAULT 0, direction TEXT NOT NULL CHECK(direction IN ('received','paid')), payment_method TEXT NOT NULL DEFAULT 'cash', date TEXT NOT NULL, reference_number TEXT, notes TEXT, ledger_expense_id INTEGER, batch_id INTEGER, payer_type TEXT NOT NULL DEFAULT 'company', payer_name TEXT, status TEXT NOT NULL DEFAULT 'posted', created_by INTEGER, created_at TEXT, updated_by INTEGER, updated_at TEXT, FOREIGN KEY(target_expense_id) REFERENCES expenses(id) ON DELETE CASCADE, FOREIGN KEY(ledger_expense_id) REFERENCES expenses(id) ON DELETE SET NULL);
        CREATE TABLE IF NOT EXISTS payment_batches (id INTEGER PRIMARY KEY AUTOINCREMENT, reference TEXT UNIQUE NOT NULL, company_name TEXT NOT NULL, person_name TEXT, direction TEXT NOT NULL CHECK(direction IN ('received','paid')), amount_original REAL NOT NULL, currency_original TEXT NOT NULL, exchange_rate_to_usd REAL NOT NULL DEFAULT 1.0, amount_base REAL NOT NULL DEFAULT 0, payment_method TEXT NOT NULL DEFAULT 'cash', date TEXT NOT NULL, reference_number TEXT, notes TEXT, allocation_mode TEXT NOT NULL DEFAULT 'oldest', allocated_amount_original REAL NOT NULL DEFAULT 0, credit_amount_original REAL NOT NULL DEFAULT 0, credit_expense_id INTEGER, payer_type TEXT NOT NULL DEFAULT 'company', payer_name TEXT, status TEXT NOT NULL DEFAULT 'posted', created_by INTEGER, created_at TEXT, updated_by INTEGER, updated_at TEXT, FOREIGN KEY(credit_expense_id) REFERENCES expenses(id) ON DELETE SET NULL);
        CREATE TABLE IF NOT EXISTS payment_allocations (id INTEGER PRIMARY KEY AUTOINCREMENT, batch_id INTEGER NOT NULL, target_expense_id INTEGER NOT NULL, payment_id INTEGER NOT NULL, amount_original REAL NOT NULL, created_at TEXT, FOREIGN KEY(batch_id) REFERENCES payment_batches(id) ON DELETE CASCADE, FOREIGN KEY(target_expense_id) REFERENCES expenses(id) ON DELETE CASCADE, FOREIGN KEY(payment_id) REFERENCES payments(id) ON DELETE CASCADE);
    ...
    cursor.executescript('''
        ...
        CREATE INDEX IF NOT EXISTS idx_direct_services_ref ON direct_services(reference);
        ...
        CREATE INDEX IF NOT EXISTS idx_payment_batches_party ON payment_batches(company_name, person_name, direction, currency_original);
        CREATE INDEX IF NOT EXISTS idx_payment_batches_date ON payment_batches(date);
        CREATE INDEX IF NOT EXISTS idx_payment_allocations_batch ON payment_allocations(batch_id);
        CREATE INDEX IF NOT EXISTS idx_payment_allocations_target ON payment_allocations(target_expense_id);
...
def ensure_db():
    ...
    if not os.path.exists(db_path):
        ...
        try:
            ...
            if 'payment_payer_type' not in cols:
                cursor.execute("ALTER TABLE expenses ADD COLUMN payment_payer_type TEXT")
            if 'payment_payer_name' not in cols:
                cursor.execute("ALTER TABLE expenses ADD COLUMN payment_payer_name TEXT")
            ...
            if 'batch_id' not in payment_cols:
                cursor.execute("ALTER TABLE payments ADD COLUMN batch_id INTEGER")
            if 'payer_type' not in payment_cols:
                cursor.execute("ALTER TABLE payments ADD COLUMN payer_type TEXT NOT NULL DEFAULT 'company'")
            if 'payer_name' not in payment_cols:
                cursor.execute("ALTER TABLE payments ADD COLUMN payer_name TEXT")
            cursor.execute("""UPDATE payments SET payer_type=CASE
                WHEN direction='paid' THEN 'office'
                WHEN COALESCE(person_name,'')<>'' THEN 'traveler'
                ELSE 'company' END
                WHERE payer_type IS NULL OR payer_type='' OR payer_name IS NULL OR payer_name=''""")
            cursor.execute("""UPDATE payments SET payer_name=CASE
                WHEN payer_type='traveler' THEN COALESCE(NULLIF(person_name,''), company_name)
                WHEN payer_type='company' THEN company_name
                ELSE COALESCE(payer_name,'') END
                WHERE payer_name IS NULL OR payer_name=''""")
            cursor.execute("CREATE TABLE IF NOT EXISTS payment_batches (id INTEGER PRIMARY KEY AUTOINCREMENT, reference TEXT UNIQUE NOT NULL, company_name TEXT NOT NULL, person_name TEXT, direction TEXT NOT NULL CHECK(direction IN ('received','paid')), amount_original REAL NOT NULL, currency_original TEXT NOT NULL, exchange_rate_to_usd REAL NOT NULL DEFAULT 1.0, amount_base REAL NOT NULL DEFAULT 0, payment_method TEXT NOT NULL DEFAULT 'cash', date TEXT NOT NULL, reference_number TEXT, notes TEXT, allocation_mode TEXT NOT NULL DEFAULT 'oldest', allocated_amount_original REAL NOT NULL DEFAULT 0, credit_amount_original REAL NOT NULL DEFAULT 0, credit_expense_id INTEGER, payer_type TEXT NOT NULL DEFAULT 'company', payer_name TEXT, status TEXT NOT NULL DEFAULT 'posted', created_by INTEGER, created_at TEXT, updated_by INTEGER, updated_at TEXT, FOREIGN KEY(credit_expense_id) REFERENCES expenses(id) ON DELETE SET NULL)")
            cursor.execute("PRAGMA table_info(payment_batches)")
            batch_cols = [c[1] for c in cursor.fetchall()]
            if 'payer_type' not in batch_cols:
                cursor.execute("ALTER TABLE payment_batches ADD COLUMN payer_type TEXT NOT NULL DEFAULT 'company'")
            if 'payer_name' not in batch_cols:
                cursor.execute("ALTER TABLE payment_batches ADD COLUMN payer_name TEXT")
            cursor.execute("""UPDATE payment_batches SET payer_type=CASE
                WHEN direction='paid' THEN 'office'
                WHEN COALESCE(person_name,'')<>'' THEN 'traveler'
                ELSE 'company' END
                WHERE payer_type IS NULL OR payer_type='' OR payer_name IS NULL OR payer_name=''""")
            cursor.execute("""UPDATE payment_batches SET payer_name=CASE
                WHEN payer_type='traveler' THEN COALESCE(NULLIF(person_name,''), company_name)
                WHEN payer_type='company' THEN company_name
                ELSE COALESCE(payer_name,'') END
                WHERE payer_name IS NULL OR payer_name=''""")
            cursor.execute("CREATE TABLE IF NOT EXISTS payment_allocations (id INTEGER PRIMARY KEY AUTOINCREMENT, batch_id INTEGER NOT NULL, target_expense_id INTEGER NOT NULL, payment_id INTEGER NOT NULL, amount_original REAL NOT NULL, created_at TEXT, FOREIGN KEY(batch_id) REFERENCES payment_batches(id) ON DELETE CASCADE, FOREIGN KEY(target_expense_id) REFERENCES expenses(id) ON DELETE CASCADE, FOREIGN KEY(payment_id) REFERENCES payments(id) ON DELETE CASCADE)")
            ...
            svc_case_cols = [c[1] for c in cursor.fetchall()]
            if 'updated_by' not in svc_case_cols:
                cursor.execute("ALTER TABLE service_cases ADD COLUMN updated_by INTEGER")
            if 'updated_at' not in svc_case_cols:
                cursor.execute("ALTER TABLE service_cases ADD COLUMN updated_at TEXT")
            if 'edit_reason' not in svc_case_cols:
                cursor.execute("ALTER TABLE service_cases ADD COLUMN edit_reason TEXT")
            ...
            cursor.execute("""UPDATE expenses SET payment_payer_type=(SELECT p.payer_type FROM payments p WHERE p.ledger_expense_id=expenses.id LIMIT 1),
                payment_payer_name=(SELECT p.payer_name FROM payments p WHERE p.ledger_expense_id=expenses.id LIMIT 1)
                WHERE source_type IN ('payment_received','payment_paid')""")
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_payments_batch ON payments(batch_id)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_payment_batches_party ON payment_batches(company_name, person_name, direction, currency_original)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_payment_batches_date ON payment_batches(date)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_payment_allocations_batch ON payment_allocations(batch_id)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_payment_allocations_target ON payment_allocations(target_expense_id)")
