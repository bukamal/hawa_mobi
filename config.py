...
def get_company_info():
    ...
    return {
        "name": _normalize_company_value(
            "name", config.get("company/name", ""), defaults["name"]
        ),
        "address": _normalize_company_value(
            "address", config.get("company/address", ""), defaults["address"]
        ),
        "phone": _normalize_company_value(
            "phone", config.get("company/phone", ""), defaults["phone"]
        ),
        "email": _normalize_company_value(
            "email", config.get("company/email", ""), defaults["email"]
        ),
        "tax_number": _clean(
            config.get("company/tax_number", ""), defaults["tax_number"]
        ),
        "logo_path": logo_path,
    }
...
def save_company_info(info):
    defaults = default_company_info()
    config = _load_config()
    config["company/name"] = _normalize_company_value(
        "name", info.get("name", ""), defaults["name"]
    )
    config["company/address"] = _normalize_company_value(
        "address", info.get("address", ""), defaults["address"]
    )
    config["company/phone"] = _normalize_company_value(
        "phone", info.get("phone", ""), defaults["phone"]
    )
    config["company/email"] = _normalize_company_value(
        "email", info.get("email", ""), defaults["email"]
    )
    config["company/tax_number"] = _clean(
        info.get("tax_number", ""), defaults["tax_number"]
    )
