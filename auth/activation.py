...
def _parse_expiration(value):
    ...
    try:
        ...
        if num > 10_000_000_000:  # milliseconds
            num = num / 1000
        return "date", datetime.datetime.fromtimestamp(
            num, datetime.timezone.utc
        ).replace(tzinfo=None)
    except Exception:
        pass
    for fmt in (
...
def activate(license_key: str) -> Tuple[bool, str]:
    ...
    try:
        resp = requests.post(
            SERVER_URL,
            json={"licenseCode": license_key, "fingerprint": device_id},
            timeout=15,
        )
...
def check_activation() -> Tuple[bool, str]:
    ...
    try:
        ...
        if (
            kind == "date"
            and parsed
            and datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)
            > parsed
        ):
...
def get_license_details() -> dict:
    ...
    try:
        ...
        exp = (
            "غير محدود"
            if kind == "lifetime"
            else (parsed.isoformat() if parsed else str(data.get("expiration") or ""))
        )
        details.update(
            {
                "expiration": exp,
