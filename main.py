...
from auth.activation import start_license_checker, stop_license_checker
...
_FIXED_DATA_DIR = os.environ.get("FLET_APP_STORAGE_DATA") or os.path.expanduser(
    "~/.hawaa"
)
...
def handle_exception(page: ft.Page, error: Exception, message: str = "خطأ غير متوقع"):
    ...
    try:
        dlg = ft.AlertDialog(
            title=ft.Text(
                "❗ خطأ", size=20, weight=ft.FontWeight.BOLD, color=ft.Colors.RED
            ),
            content=ft.Column(
                [
                    ...
                    ft.Text(
                        "راجع مخرجات الطرفية لمزيد من التفاصيل",
                        size=12,
...
def main(page: ft.Page):
    ...
    def show_splash():
        ...
        splash = SplashView(
            page=page,
            on_complete=after_splash,
            on_error=lambda msg: show_error(msg, retry=show_splash),
        )
        page.add(splash)
    ...
    def show_activation():
        ...
        activation = ActivationView(
            page=page, on_success=show_login, on_cancel=close_app
        )
        page.add(activation)
    ...
    def show_login():
        ...
        page.controls.clear()
        login = LoginView(
            page=page, on_login_success=on_login_success, on_exit=close_app
        )
        page.add(login)
    ...
    def show_change_password():
        ...
        view = MandatoryPasswordChangeView(
            page=page, on_save=show_main_app, on_cancel=show_login
        )
        page.add(view)
        page.update()
...
if __name__ == "__main__":
    sys.excepthook = lambda exctype, value, tb: print(
        f"[CRITICAL] Unhandled exception: {value}"
    )
    run_hawaa_app()
