...
class CurrencyManager:
    ...
    def get_base_currency(self) -> str:
        return self._normalize_currency(
            self._settings_repo.get("base_currency", "USD"), "USD"
        )

    def get_display_currency(self) -> str:
        return self._normalize_currency(
            self._settings_repo.get("display_currency", "USD"), "USD"
        )

    def set_base_currency(self, currency_code: str):
        self._settings_repo.set(
            "base_currency", self._normalize_currency(currency_code, "USD")
        )
        self.invalidate_cache()

    def set_display_currency(self, currency_code: str):
        self._settings_repo.set(
            "display_currency", self._normalize_currency(currency_code, "USD")
        )
        self.invalidate_cache()
    ...
    def save_runtime_settings(
        ...
        decimals=None,
        number_format=None,
        abbreviate_numbers=None,
    ):
        if base_currency is not None:
            self._settings_repo.set(
                "base_currency", self._normalize_currency(base_currency, "USD")
            )
        if display_currency is not None:
            self._settings_repo.set(
                "display_currency", self._normalize_currency(display_currency, "USD")
            )
        ...
        if abbreviate_numbers is not None:
            self._settings_repo.set(
                "abbreviate_numbers", "true" if bool(abbreviate_numbers) else "false"
            )
        self.invalidate_cache()
    ...
    def format_amount_full(
        self, amount: float, currency_code: str = None, decimals: int = None
    ) -> str:
